...
#include "Unit1.h"
#include "StackNumbr.h"
#include "Stack.h"
//---------------------------------------------------------------------------
#pragma package(smart_init)
#pragma resource "*.dfm"
TForm1 *Form1;
StackNumber *stkNumber = new StackNumber();
Stack *stk = new Stack();
float nums[5];
...
void Step(char c)
{
	float a = stkNumber->Delete();
	float b = stkNumber->Delete();
	switch(c)
	{
		case '+': stkNumber->Add(b + a); break;
		case '-': stkNumber->Add(b - a); break;
		case '*': stkNumber->Add(b * a); break;
		case '/': stkNumber->Add(b / a); break;
	}
...
void __fastcall TForm1::Button1Click(TObject *Sender)
{
	try
	{
		...
		{
			if(!is_space(node[i])) if(node[i] == '(') stk->Add('(');

			else if(node[i] == ')')
			{
				while(stk->GetBack() != '(') Step(stk->GetBack()), stk->Delete();
				stk->Delete();
			}
			...
			{
			   char c = node[i];
			   while(stk->size != 0 && priority(stk->GetBack()) >= priority(node[i])) Step(stk->GetBack()), stk->Delete();
			   stk->Add(c);
			}

			else stkNumber->Add(nums[j++]);
		}

		while(stk->size != 0) Step(stk->GetBack()), stk->Delete();
		Edit6->Text = FloatToStr(round(stkNumber->GetBack() * 1000) / 1000);
	}
	catch(...)
	{
